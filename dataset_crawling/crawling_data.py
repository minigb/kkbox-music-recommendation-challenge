# This code is an updated vesion of "https://github.com/MALerLab/pop-era-classification/blob/main/crawling_data.py"
import sys
import os
...
class MusicCrawler:
  def __init__(self, config, exclude_keywords, include_keywords, query_suffix):
    self.config = config
    self.input_csv_path = Path(config.crawling.song_list_csv_fn)
    self.save_audio_dir = Path(config.data.audio_dir)
    self.save_csv_fns = config.crawling.save_csv_fns
  ...
  def _init_save_csv_files(self):
    ...
    QUERIES_COLUMNS = [self.out_csv_col_names.title, self.out_csv_col_names.artist] + VIDEO_INFO_COLUMNS
    ...
    FAILED_COLUMNS = [self.out_csv_col_names.title, self.out_csv_col_names.artist, 'Failed Reason']
    self.failed_df = pd.DataFrame(columns=FAILED_COLUMNS)

    # chosen result df
    # if the file exists, read it and check if the columns are correct
    # if not, create an empty dataframe with the correct columns
    CHOSEN_COLUMNS = [self.out_csv_col_names.title, self.out_csv_col_names.artist] + VIDEO_INFO_COLUMNS
  ...
  def make_query(self, song, artist, exclude_keywords, topk, include_keywords):
    song_ID = self._get_song_identifier(song, artist)
  ...
  def update_result_csv(self, song_ID, queries, failed, chosen):
    title, artist = self._decode_song_identifier(song_ID)

    # queries dict
    queries_data = [{**{self.out_csv_col_names.title: title, self.out_csv_col_names.artist: artist}, **video} for video in queries]
    self.queries_df = pd.concat([self.queries_df, pd.DataFrame(queries_data)], ignore_index=True)
    
    failed_data = [{**{self.out_csv_col_names.title: title, self.out_csv_col_names.artist: artist}, **failure} for failure in failed]
    self.failed_df = pd.concat([self.failed_df, pd.DataFrame(failed_data)], ignore_index=True)
    
    if chosen:
      chosen_data = [{**{self.out_csv_col_names.title: title, self.out_csv_col_names.artist: artist}, **chosen}]
  ...
  def _remove_existing_songs_from_the_input_df(self):
    ...
    for _, row in tqdm(self.chosen_df.iterrows(), total=len(self.chosen_df)): # Get downloaded songs
      ...
      matching_row = input_df[(input_df[self.in_csv_col_names.title] == title)
  ...
  def run(self, topk):
    song_list = [(row[self.in_csv_col_names.title], row[self.in_csv_col_names.artist], self.exclude_keywords, topk, self.include_keywords) \
...
#     queries_df = queries_df.drop(columns = [self.out_csv_col_names.title, self.out_csv_col_names.artist])
...
def update_config(config):
  if not config.input_csv:
    config.input_csv = config.crawling.song_list_csv_fn
  if config.save_csv_prefix:
    # TODO(minigb): Better way to handle this?
    config.crawling.save_csv_fns.chosen = f"{config.save_csv_prefix}_chosen.csv"
    config.crawling.save_csv_fns.queries = f"{config.save_csv_prefix}_queries.csv"
    config.crawling.save_csv_fns.failed = f"{config.save_csv_prefix}_failed.csv"
...
def main(config):
  ...
  # python crawling_data.py --input_csv kpop-dataset/song_list.csv --save_csv_prefix kpop-dataset/csv/kpop --save_audio_dir audio

  # When recrawling failed music, using query suffix:
  # python crawling_data.py --save_csv_prefix csv/kpop --save_audio_dir audio --crawler_type failed --query_suffix "official audio"

  # When recrawling failed music with reuse of queries:
  # python crawling_data.py --save_csv_prefix csv/kpop --save_audio_dir audio --crawler_type reuse

  # When recrawling remastered music:
  # python crawling_data.py --input_csv csv/kpop_chosen.csv --save_csv_prefix csv/kpop_remaster --save_audio_dir audio_remastered_original --crawler_type remastered
  # python crawling_data.py --input_csv csv/kpop_chosen.csv --save_csv_prefix csv/kpop_remaster --save_audio_dir audio_remastered_original --crawler_type remastered
